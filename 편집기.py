     ...
     "fields": [
         ...
         F("output_dir", "결과 저장 폴더", "str",
           help="공유폴더는 \\\\서버이름\\... 형태로. Z: 같은 드라이브 문자는 "
                "작업 스케줄러에서 못 찾습니다."),
         F("sites.enabled", "기관 홈페이지 게시판도 읽기", "bool",
           help="나라장터에 안 올라오는 공고를 기관 입찰공고 게시판에서 직접 "
                "읽습니다. 여기서 온 공고는 무조건 'C 검토' 이고 마감일시·"
                "사업예산이 빈칸입니다. 어느 게시판을 볼지는 게시판.yaml 에 "
                "적혀 있습니다 (지금 17곳). 무엇이 잡히는지 먼저 보려면 "
                "'나라장터수집기.exe 게시판' 을 실행하세요."),
     ]},
...
def sample_rows(mod) -> list[list]:
    ...
    출처도 세 가지를 하나씩 둔다 — 나라장터 / 나라장터+사이트(한 칸에 두 줄)
    / 사이트. 기관 홈페이지 읽기를 꺼 두면 실제로는 전부 '나라장터' 로만
    나오지만, 켰을 때 표가 어떻게 보이는지를 담당자가 미리 볼 수 있어야 한다.
    '사이트' 줄은 마감일시와 예산을 비워 뒀다. 기관 게시판 목록에는 그 두
    값이 없다.
