...
그래서 실행 파일은 하나만 두고 첫 인자로 역할을 나눈다.
    ...
    나라장터수집기.exe 게시판        기관 홈페이지 게시판만 읽어 보여준다
...
USAGE = """나라장터 공고 수집기
  ...
  나라장터수집기.exe 게시판      기관 홈페이지 게시판에 무엇이 올라와 있는지
                                 보여줍니다 (수집도 메일도 하지 않습니다)
...
def 게시판보기(args: list) -> int:
    """게시판만 읽어 보여준다. 수집도 메일도 하지 않는다.

    켜기 전에 무엇이 잡히는지, 주소가 아직 살아 있는지 확인하는 용도다.
    이력에 아무것도 남기지 않으므로 몇 번을 돌려도 된다.
    """
    import 게시판
    return 게시판._main(["게시판", *args])
